...
public:
	...
	int checkvote(uint32 accountid,Player *player, Creature *_Creature)
	{
		...
		QueryResult result = WorldDatabase.PQuery("SELECT (UNIX_TIMESTAMP() - time), account, site, (SELECT count(*) FROM vote_record WHERE account = '%u'), time FROM vote_record WHERE account = '%u'",accountid, accountid);
...
class CityTelescript: public CreatureScript
{
public:
    CityTelescript() : CreatureScript("CityTelescript") { }

	bool OnGossipSelect(Player *player, Creature *_Creature, uint32 sender, uint32 action)
	{
		// Main menu
		if (sender == GOSSIP_SENDER_MAIN)
		SendDefaultMenu_CityTele(player, _Creature, action);
	...
	{
		//Go straight to the main menu
		player->ADD_GOSSIP_ITEM( 5, "Main"                    , GOSSIP_SENDER_MAIN, 5000);
		player->PlayerTalkClass->SendGossipMenu(5000,_Creature->GetGUID());
		return true;
	}

	
	void SendDefaultMenu_CityTele(Player *player, Creature *_Creature, uint32 action )
	{
	player->PlayerTalkClass->ClearMenus();
			switch(action)
			{
				case 5000:
					player->ADD_GOSSIP_ITEM( 5, "The Mall" , GOSSIP_SENDER_MAIN, 6000);
					player->ADD_GOSSIP_ITEM( 5, "Close" , GOSSIP_SENDER_MAIN, 7000);
		player->PlayerTalkClass->SendGossipMenu(5000,_Creature->GetGUID());
				break;
    ...
    struct CityTelescriptAI : public ScriptedAI
    {
       CityTelescriptAI(Creature *c) : ScriptedAI(c) {}
    };
	
    CreatureAI* GetAI(Creature* pCreature)
    {
        return new CityTelescriptAI (pCreature);
    }
};
