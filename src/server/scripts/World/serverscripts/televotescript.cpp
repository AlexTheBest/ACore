...
public:
	...
	if(!type)
	_Creature->MonsterWhisper("You have limited access, to get full access please vote on the site.", player->GetGUID());
			QueryResult result = WorldDatabase.PQuery("SELECT id,name,permissionflag,parent,folder,x,y,z,mapid,levelreq FROM telelist WHERE id = '%u' AND permissionflag='2'",action);
			if(result){
				Field* pFields = result->Fetch();
